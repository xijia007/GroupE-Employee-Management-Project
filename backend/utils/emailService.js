...
export const sendVisaStatusReminderEmail = async (to, name, nextStep) => {
  try {
    const transporter = createTransporter();
    const frontendUrl = process.env.FRONTEND_URL || "http://localhost:5173";
    const visaStatusLink = `${frontendUrl}/visaStatus`;

    const subject = "Reminder: Action Required for Your Visa Status";
    const mailOptions = {
      from: process.env.EMAIL_FROM || process.env.EMAIL_USER,
      to,
      subject,
      html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h2 style="color: #1890ff;">Visa Status - Next Step</h2>
                    <p>Hi ${name || ""},</p>
                    <p>This is a friendly reminder about your next step:</p>
                    <p><strong>${nextStep || "Please check your Visa Status page for details."}</strong></p>

                    <div style="margin: 30px 0; text-align: center;">
                        <a href="${visaStatusLink}"
                           style="background-color: #1890ff; color: white; padding: 12px 30px;
                                  text-decoration: none; border-radius: 4px; display: inline-block;">
                            Open Visa Status
                        </a>
    ...
    const info = await transporter.sendMail(mailOptions);
    console.log("✅ Visa reminder email sent successfully:", info.messageId);
    return { success: true, messageId: info.messageId };
  } catch (err) {
    console.error("❌ Visa reminder email sending failed:", err);
    throw new Error("Failed to send visa reminder email: " + err.message);
  }
};

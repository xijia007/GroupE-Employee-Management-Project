...
import { Steps, Card, Spin, message, Descriptions, Button, Tag, Image } from "antd";
...
function OnboardingApplication() {
    ...
    return (
      <Card title="📄 Your Submitted Application" style={{ marginBottom: 24 }}>
        <Descriptions bordered column={2}>
          ...
          <Descriptions.Item label="Middle Name">{applicationData.middleName || 'N/A'}</Descriptions.Item>
          <Descriptions.Item label="Preferred Name">{applicationData.preferredName || 'N/A'}</Descriptions.Item>
          <Descriptions.Item label="Email">{applicationData.email}</Descriptions.Item>
          <Descriptions.Item label="Date of Birth">{formatDate(applicationData.dateOfBirth)}</Descriptions.Item>
          <Descriptions.Item label="Gender">{applicationData.gender}</Descriptions.Item>
          <Descriptions.Item label="Cell Phone">{applicationData.cellPhone}</Descriptions.Item>
          <Descriptions.Item label="Work Phone">{applicationData.workPhone || 'N/A'}</Descriptions.Item>
          ...
          <Descriptions.Item label="US Resident Status" span={2}>
            {applicationData.usResident === 'usCitizen' ? 'US Citizen' : 
             applicationData.usResident === 'greenCard' ? 'Green Card Holder' : 
             'Work Authorization Required'}
          </Descriptions.Item>

          {applicationData.usResident === 'workAuth' && (
            <>
              <Descriptions.Item label="Visa Type">{applicationData.visaTitle}</Descriptions.Item>
              <Descriptions.Item label="Visa Period">
                {formatDate(applicationData.visaStartDate)} to {formatDate(applicationData.visaEndDate)}
              </Descriptions.Item>
            </>
          )}
        ...
        {/* Documents Section */}
        <Card type="inner" title="📎 Uploaded Documents" style={{ marginTop: 24 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {applicationData.documents?.driverLicense && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', border: '1px solid #f0f0f0', borderRadius: 8 }}>
                <span><strong>Driver's License:</strong> {applicationData.documents.driverLicense.split('/').pop()}</span>
                <div>
                  <Button 
                    icon={<EyeOutlined />} 
                    onClick={() => handlePreview(`/${applicationData.documents.driverLicense}`)}
                    style={{ marginRight: 8 }}
                  >
                    Preview
                  </Button>
                  <Button 
                    ...
                    onClick={() => handleDownload(`/${applicationData.documents.driverLicense}`, 'driver-license.pdf')}
                  >
                    Download
                  </Button>
                </div>
            ...
            {applicationData.documents?.workAuthorization && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px', border: '1px solid #f0f0f0', borderRadius: 8 }}>
                <span><strong>Work Authorization:</strong> {applicationData.documents.workAuthorization.split('/').pop()}</span>
                <div>
                  <Button 
                    icon={<EyeOutlined />} 
                    onClick={() => handlePreview(`/${applicationData.documents.workAuthorization}`)}
                    style={{ marginRight: 8 }}
                  >
                    Preview
                  </Button>
                  <Button 
                    icon={<DownloadOutlined />} 
                    onClick={() => handleDownload(`/${applicationData.documents.workAuthorization}`, 'work-authorization.pdf')}
                  >
                    Download
                  </Button>
                </div>
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {(applicationStatus === 'Approved' || applicationStatus === 'Pending') && renderApplicationDetails()}
    </div>
  );
}
